...
const MapView = ({ onSelect3D, onSelectSPB3D, initialBaseLayer = "Street" }) => {
  ...
  const [startType, setStartType]         = useState("gps")
  const [manualStartPos, setManualStartPos] = useState(null)
  const [baseLayerName, setBaseLayerName] = useState(initialBaseLayer)
  const [flyTarget, setFlyTarget]         = useState(null)

  useEffect(() => {
    if (initialBaseLayer) setBaseLayerName(initialBaseLayer)
  }, [initialBaseLayer])

  // Auto-align map onto GPS location when location is first acquired
  const hasCenteredGPS = useRef(false)
  useEffect(() => {
    if (userLocation && !hasCenteredGPS.current) {
      hasCenteredGPS.current = true
      setFlyTarget({ pos: userLocation, ts: Date.now() })
    }
  }, [userLocation])
  ...
  const panelStyle = isMobile
    ? { position:"absolute",left:10,right:10,top:10,zIndex:1000,background:"white",borderRadius:12,boxShadow:"0 2px 12px rgba(0,0,0,0.15)",overflow:"hidden",fontFamily:"Roboto,Arial,sans-serif" }
    : { position:"absolute",top:16,left:16,zIndex:1000,background:"white",padding:14,borderRadius:12,boxShadow:"0 4px 20px rgba(0,0,0,0.12)",width:360,fontFamily:"Roboto,Arial,sans-serif" }

  return (
    <div className="gm-map-shell" style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
        ...
        >
          <div className="gm-panel-content" style={{ padding: 14 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",gap:10 }}>
              <div style={{ fontWeight:600,fontSize:17,color:COLORS.ink }}>SJCET Campus Navigator</div>
            ...
            {/* Integrated Street vs Satellite Layer Switcher */}
            <div style={{ display: "flex", background: "#f1f3f4", borderRadius: 8, padding: 3, marginTop: 10, gap: 4 }}>
              <button
                type="button"
                onClick={() => setBaseLayerName("Street")}
                style={{
                  flex: 1,
                  display: "flex",
                  alignItems: "center",
                  ...
                  fontWeight: 600,
                  cursor: "pointer",
                  background: baseLayerName === "Street" ? "#ffffff" : "transparent",
                  color: baseLayerName === "Street" ? "#1a73e8" : "#5f6368",
                  boxShadow: baseLayerName === "Street" ? "0 1px 3px rgba(0,0,0,0.12)" : "none",
              ...
              <button
                type="button"
                onClick={() => setBaseLayerName("Satellite")}
                style={{
                  flex: 1,
                  display: "flex",
                  alignItems: "center",
                  ...
                  fontWeight: 600,
                  cursor: "pointer",
                  background: baseLayerName === "Satellite" ? "#ffffff" : "transparent",
                  color: baseLayerName === "Satellite" ? "#1a73e8" : "#5f6368",
                  boxShadow: baseLayerName === "Satellite" ? "0 1px 3px rgba(0,0,0,0.12)" : "none",
                  transition: "all 0.15s ease",
                }}
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
